...
mutable struct NewtonIndicesConfigurationForce{nq,nu,nc,nb,n1,n2,n3} <: NewtonIndices
    ...
    # nqlim::Int                                # box constraints on q
    # nulim::Int                                # box constraints on u
    ...
    iν::SizedArray{Tuple{n1},Int,1,1}         # implicit dynamics + box constraints lagrange multiplier
    ...
    Iν::Vector{SizedArray{Tuple{n1},Int,1,1}} # implicit dynamics + box constraints lagrange multiplier
...
function NewtonIndicesConfigurationForce(model::Model, env::Environment, H::Int)
    ...
    # nqlim = 2*nq # box constraints q
    # nulim = 2*nu # box constraints u
    # ndlim = nqlim + nulim # box constraints
    nr = nq + nu + nc + nb + nd # + ndlim # size of a one-time-step block
    ...
    iν = SizedVector{nd}(off .+ (1:(nd))); off += nd # index of the dynamics lagrange multiplier ν1 and box constraints dual
